...
violations="$({
  ...
  rg -l 'DB\.getConnection\(|DriverManager\.getConnection\(' "$SRC_DIR/services" 2>/dev/null \
    | tr '\\' '/' \
     | rg -v ' 2\.java$|/(LanApiServer|MobileItemWebServer|ServerNotificationService|ServerEmployeeScheduleService|ServerEmployeeAutoScheduleService|ServerCustomOrderDataService|ServerQuotationInvoiceService|ServerQuotationInvoiceViewService|ServerBalanceSheetService|ServerEmailOutboxService|ServerImageAssetService|ServerImageAssetMaintenance|ServerStoreSetupService|ServerStoreSwitchService|ServerFirstAdministratorService|ServerSupabaseMigrationRunner|CloudServerRegistryService|ServerManagementClient|ServerSetupGuardService|SyncWorker|SyncServiceStatusService|WorkflowSyncIdentityCollisionVerifier|ServerProvisioningService|LocalDatabaseBootstrapService|CompanyBackupService|LocalServerRepairService|.*SchemaInstaller|ReferenceDataSyncService)\.java$' || true
} | sort -u)"
