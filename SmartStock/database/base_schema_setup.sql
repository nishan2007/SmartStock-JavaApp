...
-- Read-only cache of other stores' last synchronized inventory. The local
-- store's live inventory remains authoritative and is never written here.
...
-- Read-only synchronized transaction cache and durable multistore refund workflow.
CREATE TABLE IF NOT EXISTS sync_cross_store_sales_cache (source_location_id INTEGER NOT NULL,sale_id INTEGER NOT NULL,store_name TEXT NOT NULL,receipt_number TEXT,customer_id INTEGER,user_name TEXT,payment_method TEXT,payment_status TEXT,subtotal_amount NUMERIC(14,2) NOT NULL DEFAULT 0,discount_percent NUMERIC(8,4) NOT NULL DEFAULT 0,discount_amount NUMERIC(14,2) NOT NULL DEFAULT 0,total_amount NUMERIC(14,2) NOT NULL DEFAULT 0,amount_paid NUMERIC(14,2) NOT NULL DEFAULT 0,returned_amount NUMERIC(14,2) NOT NULL DEFAULT 0,source_created_at TIMESTAMPTZ,source_updated_at TIMESTAMPTZ,cache_refreshed_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,cache_status TEXT NOT NULL DEFAULT 'CURRENT',PRIMARY KEY(source_location_id,sale_id));
CREATE INDEX IF NOT EXISTS sync_cross_store_sales_search_idx ON sync_cross_store_sales_cache(source_location_id,source_created_at DESC,receipt_number);
CREATE TABLE IF NOT EXISTS sync_cross_store_sale_items_cache (source_location_id INTEGER NOT NULL,sale_id INTEGER NOT NULL,sale_item_id INTEGER NOT NULL,product_id INTEGER NOT NULL,sku TEXT,product_name TEXT NOT NULL,product_type TEXT NOT NULL DEFAULT 'INVENTORY',quantity INTEGER NOT NULL DEFAULT 0,unit_price NUMERIC(14,2) NOT NULL DEFAULT 0,original_unit_price NUMERIC(14,2) NOT NULL DEFAULT 0,discount_percent NUMERIC(8,4) NOT NULL DEFAULT 0,discount_amount NUMERIC(14,2) NOT NULL DEFAULT 0,PRIMARY KEY(source_location_id,sale_item_id));
CREATE INDEX IF NOT EXISTS sync_cross_store_sale_items_sale_idx ON sync_cross_store_sale_items_cache(source_location_id,sale_id);
CREATE TABLE IF NOT EXISTS sync_cross_store_returns_cache (source_location_id INTEGER NOT NULL,return_id BIGINT NOT NULL,sale_id INTEGER NOT NULL,user_name TEXT,refund_method TEXT,refund_amount NUMERIC(14,2) NOT NULL DEFAULT 0,reason TEXT,source_created_at TIMESTAMPTZ,PRIMARY KEY(source_location_id,return_id));
CREATE INDEX IF NOT EXISTS sync_cross_store_returns_sale_idx ON sync_cross_store_returns_cache(source_location_id,sale_id);
CREATE TABLE IF NOT EXISTS sync_cross_store_return_items_cache (source_location_id INTEGER NOT NULL,return_item_id BIGINT NOT NULL,return_id BIGINT NOT NULL,sale_item_id INTEGER NOT NULL,product_id INTEGER NOT NULL,quantity INTEGER NOT NULL DEFAULT 0,unit_price NUMERIC(14,2) NOT NULL DEFAULT 0,PRIMARY KEY(source_location_id,return_item_id));
CREATE INDEX IF NOT EXISTS sync_cross_store_return_items_sale_item_idx ON sync_cross_store_return_items_cache(source_location_id,sale_item_id);
CREATE TABLE IF NOT EXISTS sync_cross_store_sales_status (source_location_id INTEGER PRIMARY KEY,store_name TEXT NOT NULL,row_count INTEGER NOT NULL DEFAULT 0,status TEXT NOT NULL,last_error TEXT,refreshed_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS cross_store_refund_requests (request_id UUID PRIMARY KEY,source_location_id INTEGER NOT NULL,receiving_location_id INTEGER NOT NULL,source_sale_id INTEGER NOT NULL,cloud_request_sequence BIGINT,refund_method TEXT NOT NULL,refund_amount NUMERIC(14,2) NOT NULL DEFAULT 0,reason TEXT NOT NULL,status TEXT NOT NULL,user_id INTEGER,user_name TEXT,device_id UUID,cash_drawer_id BIGINT,cash_drawer_session_id BIGINT,last_error TEXT,created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(source_location_id,source_sale_id,request_id));
CREATE INDEX IF NOT EXISTS cross_store_refund_requests_status_idx ON cross_store_refund_requests(status,created_at);
