-- SmartStock Gmail/email delivery setup.
-- Run on the cloud database before enabling store email senders.
...
ALTER TABLE email_outbox
ADD COLUMN IF NOT EXISTS attachment_body TEXT;
ALTER TABLE email_outbox
ADD COLUMN IF NOT EXISTS document_type TEXT NOT NULL DEFAULT 'MESSAGE';
ALTER TABLE email_outbox
...
ALTER TABLE email_outbox
ADD COLUMN IF NOT EXISTS queued_by_user_id INTEGER REFERENCES users(user_id);
ALTER TABLE email_outbox
...
ALTER TABLE email_outbox
DROP CONSTRAINT IF EXISTS email_outbox_status_chk;
ALTER TABLE email_outbox
ADD CONSTRAINT email_outbox_status_chk
CHECK (status IN ('QUEUED', 'SENDING', 'SENT', 'FAILED', 'CANCELLED'));

ALTER TABLE email_outbox
DROP CONSTRAINT IF EXISTS email_outbox_attempts_chk;
ALTER TABLE email_outbox
ADD CONSTRAINT email_outbox_attempts_chk
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
        ...
        REVOKE ALL ON TABLE public.email_outbox_events FROM anon;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
        REVOKE ALL ON TABLE public.email_outbox FROM authenticated;
        REVOKE ALL ON TABLE public.email_outbox_events FROM authenticated;
    END IF;
