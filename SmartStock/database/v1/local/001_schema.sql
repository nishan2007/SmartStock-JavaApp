...
CREATE TABLE public.company_customization (
    wallet_template_json text DEFAULT '' NOT NULL,
    ...
    scheduler_link_email_enabled boolean DEFAULT false NOT NULL,
    scheduler_link_notification_email text DEFAULT ''::text NOT NULL,
...
CREATE TABLE public.custom_orders (
    ...
    invoice_billed boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.customer_account_transactions (
    ...
    location_id integer,
    amount numeric(12,2) DEFAULT 0 NOT NULL,
    credit_applied_amount numeric(12,2) DEFAULT 0 NOT NULL,
...
CREATE TABLE public.sync_cross_store_customer_history_cache (
    ...
    document_balance numeric(12,2) DEFAULT 0 NOT NULL,
    note text,
    cache_refreshed_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    cache_status text DEFAULT 'CURRENT'::text NOT NULL,
...
CREATE TABLE public.custom_order_item_variant_barcodes (
...
CREATE INDEX custom_order_item_variant_barcodes_variant_idx ON public.custom_order_item_variant_barcodes(custom_variant_id);
-- Store-local runtime and credentials for the opt-in mobile item web app.
CREATE TABLE IF NOT EXISTS public.mobile_item_web_runtime (
    runtime_id smallint PRIMARY KEY DEFAULT 1 CHECK (runtime_id = 1), enabled boolean NOT NULL DEFAULT false,
    generation uuid NOT NULL DEFAULT gen_random_uuid(), changed_by_user_id integer REFERENCES public.users(user_id) ON DELETE SET NULL,
    changed_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP);
INSERT INTO public.mobile_item_web_runtime(runtime_id,enabled) VALUES(1,false) ON CONFLICT(runtime_id) DO NOTHING;
CREATE TABLE IF NOT EXISTS public.mobile_item_web_activations (
    activation_id uuid PRIMARY KEY DEFAULT gen_random_uuid(), generation uuid NOT NULL, token_hash text NOT NULL UNIQUE,
    expires_at timestamptz NOT NULL, used_at timestamptz, revoked_at timestamptz,
    created_by_user_id integer REFERENCES public.users(user_id) ON DELETE SET NULL, created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS public.mobile_item_web_browsers (
    browser_id uuid PRIMARY KEY DEFAULT gen_random_uuid(), generation uuid NOT NULL, credential_hash text NOT NULL UNIQUE,
    activated_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP, last_seen_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP, revoked_at timestamptz);
CREATE TABLE IF NOT EXISTS public.mobile_item_web_sessions (
    session_id uuid PRIMARY KEY DEFAULT gen_random_uuid(), browser_id uuid NOT NULL REFERENCES public.mobile_item_web_browsers(browser_id) ON DELETE CASCADE,
    session_hash text NOT NULL UNIQUE, csrf_hash text NOT NULL, user_id integer NOT NULL REFERENCES public.users(user_id) ON DELETE CASCADE,
    location_id integer NOT NULL REFERENCES public.locations(location_id) ON DELETE CASCADE, auth_source text NOT NULL,
    expires_at timestamptz NOT NULL, absolute_expires_at timestamptz NOT NULL, last_seen_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS public.mobile_item_web_idempotency (
    ...
    idempotency_key text NOT NULL, operation_key text NOT NULL, request_hash text NOT NULL, response_json jsonb,
    created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(browser_id,idempotency_key));
CREATE INDEX IF NOT EXISTS mobile_item_web_activation_active_idx ON public.mobile_item_web_activations(generation,expires_at) WHERE used_at IS NULL AND revoked_at IS NULL;
CREATE INDEX IF NOT EXISTS mobile_item_web_browser_active_idx ON public.mobile_item_web_browsers(generation,last_seen_at) WHERE revoked_at IS NULL;
CREATE INDEX IF NOT EXISTS mobile_item_web_session_active_idx ON public.mobile_item_web_sessions(browser_id,expires_at) WHERE revoked_at IS NULL;
...
-- Shared non-secret cloud identifiers. Private keys remain in each server's secure credential store.
...
CREATE TABLE IF NOT EXISTS public.scheduler_web_runtime (
    ...
    public_origin text,
    last_notified_origin text,
    last_notified_recipient text,
...
-- Browser identities are registered automatically after Scheduler authentication.
-- Stay-signed-in remains disabled until enabled for that browser in SmartStock.
...
ALTER TABLE public.scheduler_web_sessions ADD COLUMN IF NOT EXISTS browser_device_id uuid REFERENCES public.scheduler_web_devices(device_id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS scheduler_web_devices_user_idx ON public.scheduler_web_devices(user_id,last_seen_at DESC) WHERE revoked_at IS NULL;
CREATE INDEX IF NOT EXISTS scheduler_web_sessions_browser_idx ON public.scheduler_web_sessions(browser_device_id,expires_at) WHERE revoked_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS employee_wallet_one_active_idx ON public.employee_wallet_credentials(user_id) WHERE status='ACTIVE';
CREATE INDEX IF NOT EXISTS employee_wallet_user_idx ON public.employee_wallet_credentials(user_id,issued_at DESC);
...
CREATE INDEX IF NOT EXISTS employee_wallet_enrollment_user_idx ON public.employee_wallet_enrollments(user_id,created_at DESC);
CREATE INDEX IF NOT EXISTS employee_wallet_enrollment_location_idx ON public.employee_wallet_enrollments(location_id);
CREATE INDEX IF NOT EXISTS employee_wallet_enrollment_expiry_idx ON public.employee_wallet_enrollments(expires_at) WHERE consumed_at IS NULL;
...
CREATE INDEX IF NOT EXISTS product_lifecycle_audit_product_idx ON public.product_lifecycle_audit(product_id,created_at DESC);
...
-- WhatsApp sales-document support (server-owned; migration 20260903120000).
...
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS whatsapp_consent_by_user_id integer REFERENCES public.users(user_id);
CREATE TABLE IF NOT EXISTS public.whatsapp_configuration(location_id integer PRIMARY KEY REFERENCES public.locations(location_id),enabled boolean NOT NULL DEFAULT false,phone_number_id text,api_version text NOT NULL DEFAULT 'v23.0',message_mode text NOT NULL DEFAULT 'FULL',template_language text NOT NULL DEFAULT 'en_US',estimated_price_usd numeric(10,4) NOT NULL DEFAULT 0.05,monthly_budget_usd numeric(10,2) NOT NULL DEFAULT 5.00,template_names jsonb NOT NULL DEFAULT '{}'::jsonb,contact_line text NOT NULL DEFAULT '',updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS public.whatsapp_outbox(whatsapp_outbox_id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,location_id integer NOT NULL REFERENCES public.locations(location_id),queued_by_user_id integer NOT NULL REFERENCES public.users(user_id),customer_id integer REFERENCES public.customer_accounts(customer_id),document_type text NOT NULL,document_id bigint NOT NULL,document_number text,recipient_phone text NOT NULL,message_mode text NOT NULL,template_name text NOT NULL,message_text text NOT NULL,estimated_cost_usd numeric(10,4) NOT NULL DEFAULT 0,status text NOT NULL,meta_message_id text,last_error text,created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP);
ALTER TABLE public.whatsapp_configuration ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_outbox ENABLE ROW LEVEL SECURITY;


-- Persistent drawer count drafts and immutable lifecycle history.
CREATE TABLE IF NOT EXISTS public.cash_drawer_count_events (
 ...
 location_id integer NOT NULL REFERENCES public.locations(location_id), revision_no integer NOT NULL,
 event_type text NOT NULL CHECK(event_type IN ('COUNT_EDIT','CLEAR','FLOAT_CALCULATED','HANDOVER','CLOSE','CORRECTION')),
 denomination_counts jsonb NOT NULL DEFAULT '{}'::jsonb, float_counts jsonb NOT NULL DEFAULT '{}'::jsonb,
 counted_cash numeric(12,2) NOT NULL DEFAULT 0, float_total numeric(12,2) NOT NULL DEFAULT 0,
 cash_in_hand numeric(12,2) NOT NULL DEFAULT 0, expected_cash numeric(12,2) NOT NULL DEFAULT 0,
 complete boolean NOT NULL DEFAULT false, reason text, changed_by_user_id integer REFERENCES public.users(user_id),
 changed_by_name text, device_id uuid REFERENCES public.devices(device_id), device_name text,
 created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP,
 UNIQUE(cash_drawer_session_id,revision_no)
);
CREATE INDEX IF NOT EXISTS cash_drawer_count_events_session_time_idx ON public.cash_drawer_count_events(cash_drawer_session_id,revision_no DESC);
CREATE INDEX IF NOT EXISTS cash_drawer_count_events_location_time_idx ON public.cash_drawer_count_events(location_id,created_at DESC);
CREATE TABLE IF NOT EXISTS public.balance_sheet_drawer_sessions (
 ...
 cash_drawer_session_id bigint NOT NULL REFERENCES public.cash_drawer_sessions(cash_drawer_session_id) ON DELETE RESTRICT,
 snapshot_label text NOT NULL, snapshot_amount numeric(12,2) NOT NULL DEFAULT 0,
 PRIMARY KEY(balance_sheet_submission_id,cash_drawer_session_id)
);
CREATE INDEX IF NOT EXISTS balance_sheet_drawer_sessions_session_idx ON public.balance_sheet_drawer_sessions(cash_drawer_session_id,balance_sheet_submission_id);
CREATE OR REPLACE FUNCTION public.prevent_cash_drawer_count_event_changes() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'Cash drawer count history is immutable'; END; $$;
DROP TRIGGER IF EXISTS cash_drawer_count_events_immutable ON public.cash_drawer_count_events;
CREATE TRIGGER cash_drawer_count_events_immutable BEFORE UPDATE OR DELETE ON public.cash_drawer_count_events FOR EACH ROW EXECUTE FUNCTION public.prevent_cash_drawer_count_event_changes();
...
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS require_charge_authorization boolean NOT NULL DEFAULT false;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS sales_discount_enabled boolean NOT NULL DEFAULT false;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS sales_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS invoice_discount_enabled boolean NOT NULL DEFAULT false;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS invoice_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS custom_order_discount_enabled boolean NOT NULL DEFAULT false;
ALTER TABLE public.customer_accounts ADD COLUMN IF NOT EXISTS custom_order_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.customer_accounts DROP CONSTRAINT IF EXISTS customer_accounts_automatic_discounts_chk;
ALTER TABLE public.customer_accounts ADD CONSTRAINT customer_accounts_automatic_discounts_chk CHECK (sales_discount_percent BETWEEN 0 AND 100 AND invoice_discount_percent BETWEEN 0 AND 100 AND custom_order_discount_percent BETWEEN 0 AND 100);
ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS customer_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS customer_discount_applied boolean NOT NULL DEFAULT false;
ALTER TABLE public.quotations ADD COLUMN IF NOT EXISTS customer_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.quotations ADD COLUMN IF NOT EXISTS customer_discount_applied boolean NOT NULL DEFAULT false;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS customer_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS customer_discount_applied boolean NOT NULL DEFAULT false;
ALTER TABLE public.custom_orders ADD COLUMN IF NOT EXISTS customer_discount_percent numeric(7,4) NOT NULL DEFAULT 0;
ALTER TABLE public.custom_orders ADD COLUMN IF NOT EXISTS customer_discount_applied boolean NOT NULL DEFAULT false;
ALTER TABLE public.custom_orders ADD COLUMN IF NOT EXISTS customer_discount_amount numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE public.held_carts ADD COLUMN IF NOT EXISTS apply_customer_discount boolean NOT NULL DEFAULT true;
ALTER TABLE public.company_customization ADD COLUMN IF NOT EXISTS account_signature_retention_years integer NOT NULL DEFAULT 3;
ALTER TABLE public.company_customization DROP CONSTRAINT IF EXISTS company_customization_account_signature_retention_chk;
ALTER TABLE public.company_customization ADD CONSTRAINT company_customization_account_signature_retention_chk CHECK (account_signature_retention_years BETWEEN 1 AND 25);
CREATE TABLE IF NOT EXISTS public.customer_charge_authorizations (
    ...
    signature_purged_at timestamptz,
    sync_uuid uuid NOT NULL DEFAULT gen_random_uuid(),
    CONSTRAINT customer_charge_authorizations_evidence_chk CHECK ((signature_png IS NOT NULL AND unsigned_reason IS NULL) OR (signature_png IS NULL AND NULLIF(BTRIM(unsigned_reason),'') IS NOT NULL))
);
CREATE INDEX IF NOT EXISTS customer_charge_authorizations_customer_idx ON public.customer_charge_authorizations(customer_id,captured_at DESC);
CREATE INDEX IF NOT EXISTS customer_charge_authorizations_document_idx ON public.customer_charge_authorizations(document_type,document_id);
CREATE INDEX IF NOT EXISTS customer_charge_authorizations_expiry_idx ON public.customer_charge_authorizations(signature_expires_at) WHERE signature_png IS NOT NULL;
