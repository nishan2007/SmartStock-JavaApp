-- Custom order controls: deposits, discount reasons, and price override audit.
...
CREATE INDEX IF NOT EXISTS custom_order_lines_production_updated_by_user_fk_idx
ON custom_order_lines(production_updated_by_user_id)
WHERE production_updated_by_user_id IS NOT NULL;
...
FROM roles r
JOIN permissions p ON UPPER(p.permission_key) IN ('CUSTOM_ORDER_DEPOSIT_OVERRIDE', 'CUSTOM_ORDER_DEPOSIT_SETTINGS', 'CUSTOM_ORDER_REFUND_APPROVAL', 'CUSTOM_ORDER_REFUND_APPROVAL_SETTINGS', 'CUSTOM_ORDER_PRODUCTION_STEPS')
