    ...
    public static String searchableTextExpression(String productAlias, Integer locationId, boolean includeVendor) {
        ...
        return """
                UPPER(CONCAT_WS(' ',
                    ...
                    COALESCE(%1$s.name, ''),
                    COALESCE(%1$s.size, ''), COALESCE(%1$s.color, ''), COALESCE(%1$s.flavor, ''),
                    COALESCE((SELECT CONCAT_WS(' ',pg.name,pg.barcode,pg.additional_barcodes::text) FROM product_groups pg WHERE pg.group_id=%1$s.group_id), ''),
                    COALESCE(%1$s.variant_options::text, ''),
