...
final class LanLocationService{
    ...
    static Map<String,Object>list(Connection c,JsonObject b,int userId)throws Exception{require(c,userId);String search=text(b,"search");List<Map<String,Object>>rows=new ArrayList<>();
        String sql="""
                SELECT location_id,name,COALESCE(receipt_store_code,'0001'),COALESCE(address,''),COALESCE(company_address_line1,''),
                  ...
                  COALESCE(email_delivery_bills_enabled,FALSE),COALESCE(timezone,'America/New_York'),
                  wallet_relevance_latitude,wallet_relevance_longitude FROM locations
    ...
    static Map<String,Object>save(Connection c,JsonObject b,int userId)throws Exception{require(c,userId);Request r=parse(b);String name=required(r.name(),"Location name is required.");String code=required(r.storeCode(),"Store code is required.");
        ...
        if((r.walletLatitude()==null)!=(r.walletLongitude()==null))throw rule(400,"VALIDATION_ERROR","Enter both Wallet latitude and longitude, or leave both blank.");
        if(r.walletLatitude()!=null&&(r.walletLatitude()<-90||r.walletLatitude()>90||r.walletLongitude()<-180||r.walletLongitude()>180))throw rule(400,"VALIDATION_ERROR","Wallet coordinates are outside the valid latitude or longitude range.");
        Integer id=r.locationId();String sql=id==null?"""
                INSERT INTO locations(name,receipt_store_code,address,company_address_line1,company_address_line2,company_address_line3,company_phone_line1,company_phone_line2,
                  ...
                  email_order_confirmations_enabled,email_quotes_enabled,email_invoices_enabled,email_delivery_bills_enabled,timezone,wallet_relevance_latitude,wallet_relevance_longitude)
                ...
                UPDATE locations SET name=?,receipt_store_code=?,address=?,company_address_line1=?,company_address_line2=?,company_address_line3=?,company_phone_line1=?,company_phone_line2=?,
                  company_email_line1=?,company_email_line2=?,email_sender_address=?,email_sender_name=?,email_bcc_address=?,balance_sheet_recipient_email=?,email_receipts_enabled=?,
                  email_order_confirmations_enabled=?,email_quotes_enabled=?,email_invoices_enabled=?,email_delivery_bills_enabled=?,timezone=?,wallet_relevance_latitude=?,wallet_relevance_longitude=? WHERE location_id=? RETURNING location_id
            ...
            ps.setString(13,text(r.bccEmail()));ps.setString(14,text(r.balanceSheetEmail()));ps.setBoolean(15,r.emailReceipts());ps.setBoolean(16,r.emailOrders());ps.setBoolean(17,r.emailQuotes());ps.setBoolean(18,r.emailInvoices());ps.setBoolean(19,r.emailDelivery());ps.setString(20,r.timezone());nullableDouble(ps,21,r.walletLatitude());nullableDouble(ps,22,r.walletLongitude());if(id!=null)ps.setInt(23,id);
    ...
    private static void nullableDouble(PreparedStatement p,int i,Double value)throws SQLException{if(value==null)p.setNull(i,Types.DOUBLE);else p.setDouble(i,value);}private static Double nullableDouble(ResultSet r,int i)throws SQLException{double value=r.getDouble(i);return r.wasNull()?null:value;}
    private record Request(Integer locationId,String name,String storeCode,String address,String addressLine1,String addressLine2,String addressLine3,String phoneLine1,String phoneLine2,String emailLine1,String emailLine2,String senderEmail,String senderName,String bccEmail,String balanceSheetEmail,boolean emailReceipts,boolean emailOrders,boolean emailQuotes,boolean emailInvoices,boolean emailDelivery,String timezone,Double walletLatitude,Double walletLongitude){}
