...
public final class QuotationInvoiceService {
    ...
    public static PaymentReceiptRef recordPayment(long id,BigDecimal amount,String method,String reference)throws SQLException{JsonObject b=new JsonObject();b.addProperty("invoiceId",id);b.addProperty("amount",amount);b.addProperty("method",method);b.addProperty("reference",reference);return resultNullable("PAYMENT",b,"receipt",PaymentReceiptRef.class);}
    public static void chargeInvoiceToAccount(long id,String reason)throws SQLException{JsonObject b=new JsonObject();b.addProperty("invoiceId",id);b.addProperty("reason",reason);mutate("ACCOUNT",null,b);}
