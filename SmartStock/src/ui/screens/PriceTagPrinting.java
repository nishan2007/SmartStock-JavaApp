...
public class PriceTagPrinting extends JFrame {
    ...
    public PriceTagPrinting() {
        ...
        JPanel actions = new JPanel(new FlowLayout(FlowLayout.RIGHT, 8, 10)); JButton remove = new JButton("Remove selected"); JButton preview = new JButton("Preview selected"); JButton print = new JButton("Print tags"); actions.add(remove); actions.add(preview); actions.add(print); JPanel footer=new JPanel(new BorderLayout());footer.add(loadingState,BorderLayout.CENTER);footer.add(actions,BorderLayout.SOUTH);add(footer, BorderLayout.SOUTH);
        find.addActionListener(e -> search()); searchField.addActionListener(e -> search());UiDebouncer.bind(searchField,300,this::search); add.addActionListener(e -> addSelected()); remove.addActionListener(e -> {int row=cartTable.getSelectedRow();if(row>=0)cart.removeRow(row);}); preview.addActionListener(e -> previewSelected()); print.addActionListener(e -> printTags());
