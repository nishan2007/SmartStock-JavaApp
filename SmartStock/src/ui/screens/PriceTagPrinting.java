...
public class PriceTagPrinting extends JFrame {
    ...
    private List<CompanyCustomizationManager.PriceTagTemplateSettings> loadPriceTagTemplates() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Could not load price tag templates: " + ex.getMessage(),
                    "Price Tags", JOptionPane.ERROR_MESSAGE);
            return List.of();
        }
    }
    private void search() {
        results.setRowCount(0); String searchText=searchField.getText().trim();
        try {
            for (LanApiClient.PriceTagCatalogItem item : LanApiClient.searchPriceTagItems(searchText)) {
                results.addRow(new Object[]{item.itemType(), item.name(), item.size(), item.description(),
                        item.code(), item.price(), item.itemId()});
            }
        } catch(Exception ex){JOptionPane.showMessageDialog(this,"Could not load items: "+ex.getMessage(),"Price Tags",JOptionPane.ERROR_MESSAGE);}
    }
