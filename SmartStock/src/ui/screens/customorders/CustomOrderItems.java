...
public final class CustomOrderItems extends JFrame {
    ...
    private final DefaultTableModel itemModel=model("ID","Item","Size","Color","SKU","Department","Item Type","Brand","Pricing","Price","Variants","Qty","Reorder","Active");
    ...
    private final JTextField name=new JTextField(),size=new JTextField(),color=new JTextField(),sku=new JTextField(),barcode=new JTextField(),price=new JTextField(),quantity=new JTextField("0"),reorder=new JTextField("0"),maxWidth=new JTextField(),maxLength=new JTextField();
    ...
    private final JCheckBox variants=new JCheckBox("Track sizes / variants"),sellInPos=new JCheckBox("Sell in normal POS (without printing)"),active=new JCheckBox("Active",true);private Long itemId;
    ...
    private void saveItem(){
        try{
            ...
            String itemName=name.getText(),itemSize=size.getText(),itemColor=color.getText(),primaryBarcode=barcode.getText(),itemDescription=description.getText();
            ...
            UiTaskRunner.submit(this,"custom-catalog.save-item",()->{
                ...
                var request=new LanCustomOrderCatalogAdminService.ItemSave(targetId,itemName,itemSize,itemColor,primaryBarcode,itemDescription,imageUrl,selectedProductType,selectedPricingType,itemPrice,selectedAreaUnit,selectedDimensionUnit,width,length,hasVariants,itemQuantity,reorderLevel,isActive,categoryId,itemType,brand,sellInPos.isSelected(),codes);
    ...
    private void clearItem(){itemId=null;itemTable.clearSelection();name.setText("");size.setText("");color.setText("");sku.setText("");barcode.setText("");description.setText("");extraBarcodes.setText("");department.setSelectedDepartmentByName("Custom");classification.clearSelection();image.setImageUrl("");productType.setSelectedItem("INVENTORY");pricingType.setSelectedItem("VARIABLE");price.setText("");maxWidth.setText("");maxLength.setText("");quantity.setText("0");reorder.setText("0");variants.setSelected(false);active.setSelected(true);fieldState();}
    ...
    private void variantsDialog(){
        ...
        Rectangle usable=usableScreenBounds(this);int dialogWidth=Math.max(1,Math.min(1200,usable.width-40)),dialogHeight=Math.max(1,Math.min(720,usable.height-40));d.setSize(dialogWidth,dialogHeight);
        DefaultTableModel m=model("ID","Name","Size","Color","Brand","SKU","Barcode","Price","Qty","Reorder","Active");
        JTable t=new JTable(m);
        JTextField n=new JTextField(),variantSize=new JTextField(),variantColor=new JTextField(),variantBrand=new JTextField(),bc=new JTextField(),pr=new JTextField(),q=new JTextField("0"),re=new JTextField("0");JTextArea extraBc=new JTextArea(3,20);
        ProductImageHelper.ImageSelector img=ProductImageHelper.createImageSelector(d);
        JCheckBox a=new JCheckBox("Active",true);
        final Long[]vid={null};
        Runnable load=()->{m.setRowCount(0);var item=state.items().stream().filter(x->x.itemId()==selectedItemId).findFirst().orElse(null);if(item!=null)for(var x:item.variants())m.addRow(new Object[]{x.variantId(),x.name(),x.size(),x.color(),x.brand().isBlank()?item.brand():x.brand(),x.sku(),x.barcode(),money(x.fixedPrice()),x.quantity(),x.reorderLevel(),x.active()});};
        t.getSelectionModel().addListSelectionListener(e->{if(e.getValueIsAdjusting())return;int tableRow=t.getSelectedRow();if(tableRow<0)return;tableRow=t.convertRowIndexToModel(tableRow);vid[0]=Long.valueOf(m.getValueAt(tableRow,0).toString());var selected=state.items().stream().flatMap(i->i.variants().stream()).filter(v->v.variantId()==vid[0]).findFirst().orElse(null);if(selected!=null){n.setText(selected.name());variantSize.setText(selected.size());variantColor.setText(selected.color());variantBrand.setText(selected.brand());bc.setText(selected.barcode());extraBc.setText(String.join("\n",selected.extraBarcodes()));pr.setText(money(selected.fixedPrice()));q.setText(money(selected.quantity()));re.setText(money(selected.reorderLevel()));img.setImageUrl(selected.imageUrl());a.setSelected(selected.active());}});
        JPanel f=form();row(f,0,"Name",n);row(f,1,"Size",variantSize);row(f,2,"Color",variantColor);row(f,3,"Brand (blank inherits item)",variantBrand);row(f,4,"Barcode",BarcodeGenerationHelper.field(d,bc));JScrollPane extraScroll=new JScrollPane(extraBc);row(f,5,"More barcodes",BarcodeGenerationHelper.area(d,extraBc,extraScroll));row(f,6,"Price",pr);row(f,7,"Image",img);row(f,8,"Quantity",q);row(f,9,"Reorder",re);row(f,10,"",a);
        JButton save=new JButton("Save"),clear=new JButton("Clear"),off=new JButton("Deactivate"),phonePhoto=new JButton("Photo from Phone"),fullScreen=new JButton("Full Screen");
        final Rectangle[]normalBounds={null};
        Runnable reset=()->{vid[0]=null;t.clearSelection();n.setText("");variantSize.setText("");variantColor.setText("");variantBrand.setText("");bc.setText("");extraBc.setText("");pr.setText("");q.setText("0");re.setText("0");img.setImageUrl("");a.setSelected(true);};
        save.addActionListener(e->{
            try{
                Long variantId=vid[0];String variantName=n.getText(),savedVariantSize=variantSize.getText(),savedVariantColor=variantColor.getText(),savedVariantBrand=variantBrand.getText(),variantBarcode=bc.getText(),sourceImage=img.getImageUrl();List<String>variantExtraCodes=new ArrayList<>(new LinkedHashSet<>(List.of(extraBc.getText().split("\\R"))));variantExtraCodes.removeIf(String::isBlank);
                ...
                UiTaskRunner.submit(d,"custom-catalog.save-variant",()->{String url=ProductImageHelper.uploadLocalImageIfNeeded(sourceImage,imageNaming,"CUSTOM_VARIANT");var request=new LanCustomOrderCatalogAdminService.VariantSave(variantId,selectedItemId,variantName,savedVariantSize,savedVariantColor,savedVariantBrand,variantBarcode,url,variantPrice,variantQuantity,variantReorder,isActive,List.copyOf(variantExtraCodes));mutate("SAVE_VARIANT","variant",request);return LanApiClient.loadCustomCatalogAdmin();},loaded->{state=loaded;render();load.run();reset.run();save.setEnabled(true);off.setEnabled(true);JOptionPane.showMessageDialog(d,"Variant saved. You can add another variant now.");},failure->{save.setEnabled(true);off.setEnabled(true);error("Variant was not saved",asException(failure));});
        ...
        off.addActionListener(e->{
            ...
            UiTaskRunner.submit(d,"custom-catalog.deactivate-variant",()->{LanApiClient.updateCustomCatalogAdmin("DEACTIVATE_VARIANT",body,UUID.randomUUID().toString());return LanApiClient.loadCustomCatalogAdmin();},loaded->{state=loaded;render();load.run();reset.run();save.setEnabled(true);off.setEnabled(true);},failure->{save.setEnabled(true);off.setEnabled(true);error("The variant was not deactivated",asException(failure));});
        });
        phonePhoto.addActionListener(e->{Long variantId=vid[0];if(variantId==null){JOptionPane.showMessageDialog(d,"Save or select a variant first.");return;}showPhonePhoto("variant",variantId);});
        fullScreen.addActionListener(e->{if(normalBounds[0]==null){normalBounds[0]=d.getBounds();d.setBounds(usableScreenBounds(d));fullScreen.setText("Restore");}else{d.setBounds(normalBounds[0]);normalBounds[0]=null;fullScreen.setText("Full Screen");}});
        JPanel buttons=new JPanel(new GridLayout(0,2,6,6));buttons.add(save);buttons.add(clear);buttons.add(off);buttons.add(phonePhoto);buttons.add(fullScreen);row(f,11,"",buttons);
        JSplitPane split=new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,new JScrollPane(t),new JScrollPane(f));split.setResizeWeight(.6);d.add(split);load.run();d.setLocationRelativeTo(this);split.setDividerLocation(.6);d.setVisible(true);refresh();
    }
    ...
    private void showPhonePhoto(String targetType,long targetId){
        UiTaskRunner.submit(this,"custom-catalog.phone-photo-create",()->LanApiClient.createMobileItemPhotoHandoff(targetType,targetId),handoff->showPhonePhotoDialog(handoff),failure->error("The phone photo link could not be created",asException(failure)));
    }
    private void showPhonePhotoDialog(LanApiClient.MobileItemPhotoHandoff handoff){
        JDialog dialog=new JDialog(this,"Photo from Phone",false);dialog.setDefaultCloseOperation(DISPOSE_ON_CLOSE);dialog.setSize(440,560);dialog.setLocationRelativeTo(this);
        JPanel panel=new JPanel(new BorderLayout(10,10));panel.setBorder(new EmptyBorder(16,16,16,16));JLabel heading=new JLabel("Scan with your phone",SwingConstants.CENTER);heading.setFont(heading.getFont().deriveFont(Font.BOLD,20f));panel.add(heading,BorderLayout.NORTH);
        JLabel qr=new JLabel(new ImageIcon(qrCode(handoff.url(),330)),SwingConstants.CENTER);panel.add(qr,BorderLayout.CENTER);JLabel status=new JLabel("<html><center>Photo for <b>"+html(handoff.targetName())+"</b><br>This link expires in 10 minutes.</center></html>",SwingConstants.CENTER);panel.add(status,BorderLayout.SOUTH);dialog.setContentPane(panel);
        Timer timer=new Timer(1800,null);final boolean[]busy={false};timer.addActionListener(e->{if(busy[0]||!dialog.isDisplayable())return;busy[0]=true;UiTaskRunner.submit(dialog,"custom-catalog.phone-photo-status",()->LanApiClient.mobileItemPhotoHandoffStatus(handoff.handoffId()),result->{busy[0]=false;if(result.uploaded()){timer.stop();status.setText("Photo uploaded successfully. You may close this window.");SessionDataCache.invalidate("custom-catalog:");refresh();}},failure->{busy[0]=false;});});dialog.addWindowListener(new java.awt.event.WindowAdapter(){@Override public void windowClosed(java.awt.event.WindowEvent e){timer.stop();}});timer.start();dialog.setVisible(true);
    }
    private static java.awt.image.BufferedImage qrCode(String value,int size){try{var matrix=new com.google.zxing.qrcode.QRCodeWriter().encode(value,com.google.zxing.BarcodeFormat.QR_CODE,size,size);var image=new java.awt.image.BufferedImage(size,size,java.awt.image.BufferedImage.TYPE_INT_RGB);for(int y=0;y<size;y++)for(int x=0;x<size;x++)image.setRGB(x,y,matrix.get(x,y)?Color.BLACK.getRGB():Color.WHITE.getRGB());return image;}catch(Exception e){throw new IllegalArgumentException("The phone photo QR code could not be created.",e);}}
    private static Rectangle usableScreenBounds(Component component){GraphicsConfiguration configuration=component.getGraphicsConfiguration();if(configuration==null)configuration=GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration();Rectangle bounds=configuration.getBounds();Insets insets=Toolkit.getDefaultToolkit().getScreenInsets(configuration);return new Rectangle(bounds.x+insets.left,bounds.y+insets.top,Math.max(1,bounds.width-insets.left-insets.right),Math.max(1,bounds.height-insets.top-insets.bottom));}
